...
def evaluate_model_on_tests(
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_dataloaders))):
            ...
            for (X, y, sens) in test_dataloader_iterator:
                if torch.cuda.is_available() and use_gpu:
                    X = X.cuda()
                    y = y.cuda()

                y_pred = model(X).detach().cpu() # tensor
                y = y.detach().cpu()

                sens_final.append(sens.numpy())
                y_pred_final.append(y_pred.numpy()) # list of arrays
                y_true_final.append(y.numpy())

            sens_final = np.concatenate(sens_final)
            y_true_final = np.concatenate(y_true_final)
            y_pred_final = np.concatenate(y_pred_final)
            results_dict[f"client_test_{i}"] = metric(y_true_final, y_pred_final)

            new_sens_final = sens_final.squeeze()
            new_y_true_final = y_true_final.squeeze()
            new_y_pred_final = y_pred_final.squeeze() >= 0.5
            # print(sens_final.shape, y_true_final.shape, y_pred_final.shape)
            print(new_sens_final.shape, new_y_true_final.shape, new_y_pred_final.shape)
            # print(new_y_pred_final)

            pred_acc = new_y_pred_final == new_y_true_final
            print('TN TEST pred_acc: ', pred_acc)

            for s_value in np.unique(new_sens_final):
                indexs0 = np.logical_and(new_y_true_final == 0.0, new_sens_final == s_value) # y=0
                indexs1 = np.logical_and(new_y_true_final == 1.0, new_sens_final == s_value) # y=1

                tnr_list.append(np.mean(pred_acc[indexs0]))
                tpr_list.append(np.mean(pred_acc[indexs1]))
                print('tnr_list', tnr_list)
    ...
    pe_gap = max(tnr_list) - min(tnr_list)
    print('EO, PE: ', eo_gap, pe_gap)
