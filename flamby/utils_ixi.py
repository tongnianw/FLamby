...
def evaluate_model_on_tests(
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_dataloaders))):
            ...
            # Create a boolean mask where sens_final sex=1, age>=65
            mask1 = sens_final >= 60
            y_true_1 = y_true_final[mask1]
            y_pred_1 = y_pred_final[mask1]
            results_dict[f"client_test_{i}_sex_1"] = metric(y_true_1, y_pred_1)

            # Create a boolean mask where sens_final sex=2, age>65
            mask2 = sens_final < 60
            y_true_2 = y_true_final[mask2]
            y_pred_2 = y_pred_final[mask2]
            results_dict[f"client_test_{i}_sex_2"] = metric(y_true_2, y_pred_2)
